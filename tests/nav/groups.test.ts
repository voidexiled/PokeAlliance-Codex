...
describe('WG5: a link exists only if its route exists in the build', () => {
  ...
  it('fills «Actividades» from the registry, in its order (8.0.3, 8.9)', () => {
    ...
    for (const locale of ['es', 'en'] as const) {
      ...
      // Only «Destacados» carries sprites (7.10.2).
      expect(group?.items.every((link) => link.sprite === undefined)).toBe(true);
    }
...
describe('«Destacados»: the pinned group of content/destacados.json (8.0.3, 7.10.2)', () => {
  ...
  it('gives each entry the sprite of its record at rest, and the Diamond alone at half (R11)', () => {
    ...
    for (const [index, entry] of destacados.entries()) {
      ...
      expect(link?.half, entry.ruta).toBe(entry.sprite === 'ui/diamond' ? true : undefined);
    }
    // No other group carries sprites (7.10.2).
    const nav = buildNav('es', '/es/');
    const others = [...nav.top, ...nav.groups.filter((g) => !g.pinned).flatMap((g) => g.items)];
    expect(others.every((link) => link.sprite === undefined && link.half === undefined)).toBe(true);
  });
...
describe('Sidebar.astro: the sprites of the pinned group on whole pixels (DS:Sidebar, §3.7)', () => {
  ...
  const pinned = (sprites: [string, SidebarLink['sprite'], true?][]): SidebarNav => ({
    top: [],
    groups: [
      {
        id: 'destacados',
        ...
        pinned: true,
        open: false,
        items: sprites.map(([id, sprite, half]) => ({
          id,
          label: id,
          href: `/es/${id}/`,
          sprite,
  ...
  it('names the odd sides in `data-snap` and keeps lengths out of `style`', async () => {
    ...
    // 16 × 16 and the Diamond at half (16 × 16) are even; the box without a sprite has none.
    expect(boxes).toEqual([null, 'x y', 'y', 'x', null, null]);
